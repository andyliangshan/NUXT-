<template>
    <div>
        ...
        <dialog class="submitting-dialog" :open="isSubmitting">{{submitInfo}}</dialog>
    </div>
</template>
...
export default {
  ...
  methods: {
    ...
    publish: async function() {
      ...
      const categoryId = location.pathname.match(/\w{8}-(\w{4}-){3}\w{12}/)[0];
      const resp = await axios.post('/api/tweet/put', { content, images, categoryId });
      //   const resp = { success: true };
      if (resp.data.success) {
        this.submitInfo = '发布成功';
